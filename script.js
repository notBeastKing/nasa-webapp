document.addEventListener('DOMContentLoaded', () => 
    {
        ...
        {
            let angle = startAngle; // Use the provided start angle
            let isPaused = false; // Pause state for each individual planet
            ...
            function update() {
                ...
                if (!isPaused) {
                    ...
                    // Center the container (planet) on its orbit by adjusting its transform
            ...
            // Stop only this planet's animation when hovered
            container.addEventListener('mouseenter', () => {
                isPaused = true;
                cancelAnimationFrame(currentRequest);
                showTooltip(container.querySelector('.planet')); // Show planet name
            });
    
            // Resume only this planet's animation when the mouse leaves
            container.addEventListener('mouseleave', () => {
                isPaused = false;
                update();
                hideTooltip(); // Hide planet name
            });
        }
        ...
        function openPlanetModal(planetName, planetData) 
        {
            ...
            // Set planet-specific background colors
            const planetStyles = {
                "Mercury": {
                    color: "135deg, #000000,#000000, #434343", // Gray for Mercury
                    image: "assets/mercury_modal/mercurysurf.png"
                },
                "Venus": {
                    color: "180deg, #000000,#000000,#000000,#000000,#9c7f6a,#b56c4c", // Golden for Venus
                    image: "assets/venus_modal/venus_surf.png"
                },
                "Earth": {
                    color: "170deg, #000000,#000000,#000000,#8abbcf,#4c61b5", // Blue for Earth
                    image: "assets/earth_modal/earth.png"
                },
                "Mars": {
                    color: "150deg, #000000,#000000,#9c7f6a", // Reddish for Mars
                    image: "assets/mars_modal/marsurf.png"
                },
                "Jupiter": {
                    color: "180deg, #000000,#000000,#000000,#000000, #a59186,#c99039", // Light Brown for Jupiter
                    image: "assets/jupiter_modal/jupitersurf.png"
                },
                "Saturn": {
                    color: "175deg, #000000,#000000,#000000,#000000,#cecece,#bfbdaf", // Yellowish for Saturn
                    image: "assets/saturn_modal/saturnsurf.png"
                },
                "Uranus": {
                    color: "175deg, #000000,#000000,#000000,#85addb", // Light Blue for Uranus
                    image: "assets/uranus_modal/urannusurf.png"
                },
                "Neptune": {
                    color: "175deg, #000000,#000000,#000000,#85addb,#274687", // Dark Blue for Neptune
            ...
            const selectedPlanet = planetStyles[planetName] || { color: "#fff", image: "" };
            // Insert the planet information (dynamic)
            ...
            <p class="animated-text" style="--delay: 0.1s;">${planetData.info1}</p>
            <p class="animated-text" style="--delay: 0.2s;">${planetData.info2}</p>
            <p class="animated-text" style="--delay: 0.3s;">${planetData.info3}</p>
            <p class="animated-text" style="--delay: 0.4s;">${planetData.info4}</p>
            <p class="animated-text" style="--delay: 0.5s;">${planetData.info5}</p>`;
    
        
        
            // Change the background color of the modal based on the planet
        ...
        function getPlanetInfo(planetName) 
        {
            const planetData = {
               "Mercury": {
                    ...
                    "info5": "                           ",
                    "gif": "assets/mercury_modal/mercury-ezgif.com-resize.gif"
                },
     ...
     "Neptune": {             
            ...
            return planetData[planetName] || {info: "Information about this planet is not available.", img: ""};
        }
        ...
        // Grabbing the planet containers by their IDs
        ...
        // Move the planets in elliptical orbits around the center (with slower speeds)
        // Updated to match the new smaller semiMajorAxis and semiMinorAxis values
    ...
    // Function to create stars as a background effect
function createStarfield(numStars) {
    ...
    for (let i = 0; i < numStars; i++) {
        ...
        const size = Math.random() * 2 + 1; // Stars between 1px and 3px
        const x = Math.random() * window.innerWidth;
        const y = Math.random() * window.innerHeight;
        const opacity = Math.random() * 0.8 + 0.2; // Opacity between 0.2 and 1
  
        // Apply CSS styles to the star
...
// Function to create a random asteroid with size between 1px to 4px
function createAsteroid() {
    const asteroid = document.createElement('div');
    asteroid.classList.add('asteroid');
  
    // Set random size for the asteroid, now between 1px and 4px
    const size = randomInt(1, 4); // Reduced size between 1px and 4px
...
// Function to generate multiple layers of asteroid belts that rotate around the sun
function generateAsteroidBelt(layers, asteroidsPerLayer) {
    const container = document.querySelector('.solar-syst');
  
    const baseRadius = 200; // Smaller base radius to bring asteroids closer to the center
    const layerSpacing = 4; // Reduce spacing to make layers tightly packed
    const centerX = window.innerWidth / 2;
    const centerY = window.innerHeight / 2;
  
    for (let layer = 0; layer < layers; layer++) {
        const currentRadius = baseRadius + layer * layerSpacing;  // Radius grows with each layer

        // Create a container for each layer to apply rotation animation
        ...
        layerContainer.style.animationDuration = `${randomInt(50, 120)}s`; // Random speed for each layer

        for (let i = 0; i < asteroidsPerLayer; i++) {
            const asteroid = createAsteroid();
  
            // Calculate a random angle for each asteroid to position in a circular orbit
            const angle = Math.random() * 2 * Math.PI;

            // Add slight variation to the orbit to make paths non-circular
            const randomOffset = randomInt(-10, 10);
            const orbitX = (currentRadius + randomOffset) * Math.cos(angle);
            const orbitY = (currentRadius + randomOffset) * Math.sin(angle);
  
            // Set the asteroid's position within the layer container
            asteroid.style.left = `${orbitX + currentRadius}px`;
            asteroid.style.top = `${orbitY + currentRadius}px`;
  
            // Append asteroid to the layer container
            layerContainer.appendChild(asteroid);
        }

        // Append each layer container to the solar system
...
// Call the functions to generate stars and multi-layer asteroid belt on page load
window.onload = function() {
    createStarfield(300);              // Generate 300 stars
    generateAsteroidBelt(10, 20);      // Generate 7 layers, each with 100 tightly packed asteroids
...
document.querySelectorAll('.asteroids-container').forEach(asteroid => {
  asteroid.addEventListener('click', function() {
    window.location.href = 'https://example.com'; // Replace with the desired website
  });
});
...
function moveAsteroid(container, speed) {
  ...
  // Define the area where asteroids can be placed (80% of the screen)
  const maxX = screenWidth * 0.8;
  const maxY = screenHeight * 0.8;

  // Randomly place the asteroid within 80% of the screen
  let x = Math.random() * maxX;
  let y = Math.random() * maxY;
  
  let deltaX = (Math.random() - 0.5) * speed; // Slow random movement in X direction
  let deltaY = (Math.random() - 0.5) * speed; // Slow random movement in Y direction
...
moveAsteroid(ast1, 10);
moveAsteroid(ast2, 10);
moveAsteroid(obj1, 10);
moveAsteroid(obj2, 10);
moveAsteroid(comet1, 10);
moveAsteroid(comet2, 10);